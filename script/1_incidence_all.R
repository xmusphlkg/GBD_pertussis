...
cat("Global incidence: ", sum(data_trend_number$val), "\n")
...
cat("Joinpoint APC for incidence: ", jp_apc$apc, "\n")
print(jp_apc)

cat("Joinpoint for incidence: ", "\n")
print(data_panel_A[data_panel_A$year %in% c(jp_apc$segment_start, jp_apc$segment_end),])

cat("Joinpoint for cases: ", "\n")
print(data_trend_number[data_trend_number$year %in% c(jp_apc$segment_start, jp_apc$segment_end),])

cat("Decrease in cases: ", data_trend_number$val[data_trend_number$year == 2021] - data_trend_number$val[data_trend_number$year == 1990],
...
fig1 <- ggplot(data_panel_A, aes(x = year, y = val, group = 'A')) +
     geom_point(mapping = aes(color = 'Incidence')) +
     geom_linerange(aes(ymin = lower, ymax = upper, color = 'Incidence'),
                    show.legend = FALSE) +
     ...
     scale_color_manual(values = c(Incidence = "#E84A5FFF",
                                   `1` = "#99B898FF",
                                   `2` = "#FECEA8FF",
                                   `3` = "#019875FF"),
     ...
     guides(color = guide_legend(override.aes = list(linetype=c(1,1,1,1), shape=c(NA,NA,NA,19))))
...
ggsave(filename = "outcome/fig1.pdf",
       plot = fig,
       width = 12,
       height = 9,
       device = cairo_pdf,
